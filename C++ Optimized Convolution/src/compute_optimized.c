  ...
  for (int i = 0; i < rows_c; i++) {
      for (int j = 0; j < cols_c; j++) {
          for (int k = 0; k < rows_b; k++) {
              ...
              for (int b = 0; b < (cols_b/40) * 40; b += 40) {
                  ...
                  vec_a = _mm256_loadu_si256((__m256i *)(data_a + ((i + k) * cols_a + j + b+32)));
                  vec_b = _mm256_loadu_si256((__m256i *)(data_b + (k * cols_b + b+32)));
                  mult = _mm256_mullo_epi32(vec_a, vec_b);
                  sum_vec = _mm256_add_epi32(sum_vec, mult);
              }
              for (int m = (cols_b/40)*40; m < (cols_b/8) * 8; m += 8) { 
                  ...
                  __m256i mult = _mm256_mullo_epi32(vec_a2, vec_b2);
                  sum_vec = _mm256_add_epi32(sum_vec, mult);
              }
              int32_t tmp_arr[8];
